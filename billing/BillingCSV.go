	...
	if err == nil {
		...
		for {
			...
			if cnt > 1 {
				...
				rcli, ok1 := billingModels.ReportingCategoryLookup.Get(l.MeterCategory, l.MeterSubCategory)
				if ok1 {
				...
				pmi, ok3 := billingModels.MeterLookup.Get(l.MeterId)
				if ok3 {
				...
				scli, ok4 := billingModels.SummaryCategoryLookup.Get(cat, subcat, uom, l.MeterCategory, l.MeterSubCategory)
				if ok4 {
					summaryCategory = scli.Summary
					quantityDivisor = scli.QuantityDivisor
					divisor = billingModels.SummaryCategoryLookup.GetDivisor(quantityDivisor, l.BillingPeriodEndDate)
				} else {
				...
				if l.MeterCategory == "Storage" && len(l.MeterName) > 6 {
					...
					if l.MeterName[len(l.MeterName)-6:] == " Disks" {
						...
						mdli, ok4 := billingModels.ManagedDiskLookup.Get(l.MeterName)
				...
				*/
				...
				if cat == "IaaS" && subcat == "Compute" && uom == "1 Hour" && pmi.ArmSkuName != "" {
					...
					if ok3 {
						...
						vmli, ok5 := billingModels.VmSizeLookup.Get(pmi.ArmSkuName)
		...
		outputAggregateRGCsvFile := config.ConfigMap.WorkingDirectory + config.ConfigMap.OutputAggregateRGCsvFile
